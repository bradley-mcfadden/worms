...
export(Dictionary) var ent_state_prop = {
	EntityState.PATROL: {color = Color.aquamarine, speed = 250, threshold = 32, fov = 90},
	EntityState.CHASE: {color = Color.crimson, speed = 350, threshold = 200, fov = 360},
	EntityState.DEAD: {color = Color.black, speed = 0, threshold = 0, fov = 0},
}
...
func _physics_process(delta: float):
	...
	match current_state:
		EntityState.CHASE:
			...
			var did_attack = _check_melee_attack(dist_to_player)
			if did_attack:
				return
			did_attack = _check_ranged_attack(dist_to_player)
		EntityState.PATROL:
			pass
		EntityState.DEAD:
			pass
...
func _set_interest():
	...
	var path_target = _get_target()
	# print(path_target, " " , position)
	if path_target == null:
		_set_default_interest()
		return SeekState.NO_TARGET
	var space_state := get_world_2d().direct_space_state
	var result := space_state.intersect_ray(position, path_target, [self])
	# print(collision_layer)
	var distance_to = global_position.distance_to(path_target)
	if distance_to and (result.has("collider") and not result["collider"] is StaticBody2D):
...
func _check_ranged_attack(dist_to_player):
...
func start_ranged_attack():
	...
	new_bullet.setup(
		self,
		(target - global_position).normalized() * 1000,
		ranged_damage,
		10,
		collision_layer,
		layer
...
func _check_melee_attack(dist_to_player):
	if !has_melee_attack:
		return false
